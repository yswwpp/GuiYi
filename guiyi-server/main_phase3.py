...
from adapters.feishu_adapter import FeishuAdapter, FeishuAccount
...
from sync.metadata import SyncMetadata
from sync.engine import SyncEngine
from sync.scheduler import SyncScheduler
from ai.ai_service import AIService
from ai.config_store import AIConfigStore
from ai.rerank_service import RerankConfig, create_reranker
from ai.keyword_extractor import KeywordExtractorConfig, create_keyword_extractor
from adapters.yinxiang_adapter import YinxiangAdapter, YinxiangAccount
...
def get_index():
    ...
    if index is None and not index_loaded:
        try:
            ...
            index = IndexManager()
            index_loaded = True
            logger.info(f"✓ 向量索引加载成功，文档数: {index.embeddings.count()}")
...
async def root():
    ...
    return {
        ...
        "version": "0.3.0",
        "docs": "/docs",
        "features": ["web", "feishu_multi_account", "local_files", "incremental_sync", "scheduled_sync"],
...
async def get_sync_stats():
    ...
    try:
        ...
        # 补充未同步的数据源
        all_sources = ["feishu", "local", "yinxiang", "quark", "web"]
...
async def add_yinxiang_account(req: YinxiangAccountRequest):
    ...
    try:
        ...
        # 保存账号名到配置文件
        config_path = os.path.join(os.path.dirname(__file__), 'data', 'yinxiang_accounts.json')
        account_list = []
